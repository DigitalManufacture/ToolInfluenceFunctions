...
sig_a=0.01; %planar radius of aspherity, mm
...
for i=1:length(x)
    for j=1:length(y)
        if x(i)^2+y(j)^2<r1^2
           ...
           Pij(i,j)=1/b1*Er^(2/3)*Pxy(i,j)^(1/3)/1e6;%MPa
           N(i,j)=G*(pi*l_a*(3*sig_a^2+l_a^2)/6);
           Fgxy(i,j)=Pxy(i,j)*pi*(dg/2)^2;
           if Fgxy(i,j)<=F_cri
               h(i,j)=pH_Cal(sigmay,dg,Fgxy(i,j));
               As(i,j)=4/3*sqrt(2)*sqrt(dg/2)*(h(i,j))^(3/2);
           else
               [CL,CH]=CLCH_Cal(kf,H,semi_angle,Ew,miu_w,Fgxy(i,j));
               ch(i,j)=CH; cl(i,j)=CL;
               As(i,j)=2*CL*CH;
           end
           ...
           MRR_th(i,j)=MRR(i,j)/pi/(da(i,j)*R_a);
