...
Ep=33.3e6; %500e6*5 resin for test, young's modulus of polishing pad, polyuthan
% Ep=29e6; %500e6*5 resin for test, young's modulus of polishing pad, uninap
...
H=800*9.8*1e6; %viker hardness in Pa  http://www.dynacer.com/properties/hardness/
c=3; %yield stress=H/c
tensile=8000e6; %workpiece tensile stress in PA  
...
l_a=0.0108; %asperity height,mm, LP66
% l_a=0.0068; %asperity height,mm, uninap
% sig_a=0.03; %planar radius of aspherity, mm
% A_a=pi*sig_a^2;
% R_a=(sig_a^2+l_a^2)/2/l_a;
% R_a=0.04;
sig_a=0.052; %planar radius of aspherity, mm, for LP66
% sig_a=0.03; %planar radius of aspherity, mm, for uniap
R_a=(sig_a^2+l_a^2)/2/l_a;
% l_a=R_a-sqrt(R_a^2-sig_a^2);
% R_a=0.17; l_a=0.02; sig_a=0.08;
A_a=pi*sig_a^2;
A_a=4*sig_a^2;
...
[F_cri,p_cri,h_cri,p_cri_elastic,h_cri_elastic]=Critical_Cal(alpha,nt,theta,kf,H,dg,miu_w,Ew,miu_grit,Egrit,tensile);
[trap_depth trap_pressure]=trap_depth_Cal(H,dg,Ep,Hp,Cal_k_coe(H,dg,miu_w,Ew));
...
for i=1:length(x)
    for j=1:length(y)
        if x(i)^2+y(j)^2<r1^2
           ...
           %N(i,j)=G*pi*r1*r1*0.04/2200*0.058; %estimate from area ratio 4%
           N(i,j)=G*bb(i,j)*A_a*l_a;
           %N(i,j)=G*A_a*l_a;
           Fgxy(i,j)=Pij(i,j)*pi*(dg/2)^2;
           if Pij(i,j)<=trap_pressure
              Fgxy(i,j)=Pij(i,j)*pi*(dg/2)^2; 
           else
              Fgxy(i,j)=H/1e6/c*pi*(dg/2)*trap_depth;
           end
%            Fgxy(i,j)=Pij(i,j)*pi*(dg/2)^2; 
           if Fgxy(i,j)<=F_cri 
           ...
           MRR_th(i,j)=MRR(i,j)/(A_a)/1;
           %MRR_th(i,j)=MRR(i,j)/(pi*r1*r1/2200)/1; %area ratio method
        end
    end
end
[X,Y]=meshgrid(x,y);
Z=-MRR_th;
[h_cri_elastic h_cri]
[ max(max(Pxy)) p_cri_elastic max(max(Pij)) trap_pressure p_cri]
mesh(X,Y,Z*5)

MatVol=0;dxy=x(2)-x(1); 
for i=1:length(x)
for j=1:length(x)
if ~isnan(MRR(i,j)) 
MatVol=MatVol+MRR(i,j);
end
end
end
MatVol*5 % 5s

%% Preston Coe Prediction
MatVol=0;dxy=x(2)-x(1); %/s
for i=1:length(x)
for j=1:length(x)
if ~isnan(MRR(i,j)) 
MatVol=MatVol+MRR(i,j);
...
for i=1:length(Pxy(:,1))
    for j=1:length(Pxy(:,1))
        if ~isnan(Vr(i,j)) && (~isnan(Pxy(i,j)) )
            MatVolForK=MatVolForK+Pxy(i,j)*Vr(i,j)*dxy*dxy*1e6;
        end
    end
end
K=MatVol/MatVolForK  %%unit:1/Pa
K_exp=0.0144/12/5/MatVolForK %5s  LP66
%K_exp=0.000768/5/MatVolForK %5s  LP66 400rpm
% K_exp=0.0074/12/5/MatVolForK %5s  uninap
% K_exp=0.00672/12/5/MatVolForK %5s  90A smooth
